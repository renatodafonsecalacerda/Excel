...
var kb_tabs_minusimgAlt = "Collapse image";
var kb_tabs_plusimgAlt = "Expand image";
...
function tocScrollTo(e) {
    ...
    $(e.scrollNode).find('a').focus();
    return false;
}

function makeTOCNode(HNode, depth, type) {
    var n = document.createElement("li");

    var t = getInnerText(HNode);
    n.innerHTML = "<a bi:title=\"" + type + "\" href=\"#\" onclick=\"return tocScrollTo(this);\"><img src=\"" + tocArrow + "\" alt=\"\" /><span class=\"tocTxt\">" + t + "</span></a><ul class=\"tocLine\"></ul>";
...
var loadTOCNode = function (depth, type) {
    ...
    if (passesTypeFilter(type) && depth <= depthLimit) {
        kbHeadings.push(e);
        var c = makeTOCNode(e, depth, type);
...
function kb_Collapseall(e) {
    ...
    $($(e).children(".kb_tabs_toggle")).attr("alt", kb_tabs_plusimgAlt);
}
...
var kb_onload = function () {
    ...
    kbtoggle.alt = kb_tabs_minusimgAlt;
...
function DisplayTranslatePanel() {
    if ($("#kb_translatePanel").is(":visible")) {
        $("#kb_translatePanel").hide();
        $("#kb_translateBtn img").attr("src", kb_translateiconurl);
    } else {
        $("#kb_translatePanel").show();
        $("#kb_translateBtn img").attr("src", kb_translateexpandiconurl);
    }
}