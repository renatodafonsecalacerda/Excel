...
MS.Support.AC.prototype =
{
    ...
    OutputSuggestions: function (query, output) {
        try {
            ...
            $('#searchAssertiveDiv').html($('#acDropdownOpenMessage').val());
        }
        catch (ex) {
            this.ACHideTarget();
        }
        ...
        if (this.options.acListDiv && this.options.acListDiv.style && this.options.acListDiv.style.visibility != "hidden") {
            ...
            if (k == 40) {
                ...
                if (si < 0) {
                    ...
                    this.PopBubble();
                    $('#searchAssertiveDiv').html($('#gsfx_bsrch_query').val() + ' ' + $('#acDropDownSelected').val());
                }
                else {
                    ...
                    if (currentText != null) {
                        ...
                        q.value = currentText;
                        $('#searchAssertiveDiv').html($('#gss_ac_dtxt' + si.toString()).html() + ' ' + $('#acDropDownSelected').val());
                    }
        ...
        for (var i = 0; i < sibs.length; i++) {
            if (sibs[i].className == 'acSugDiv') {
                if (i != this.options.acQueryBox.selectedIndex) {
                    ...
                    this.HighlightEls(sibs[i], 'text-decoration', 'none');
                }
                else {
                    this.HighlightEls(sibs[i], 'background', this.options.clrhigh);
                    this.HighlightEls(sibs[i], 'color', this.options.clrhightxt);
                    this.HighlightEls(sibs[i], 'text-decoration', this.options.clrhightxtunderline);
                }
            }
        }
    },
