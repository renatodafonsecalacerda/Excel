...
if (!MS.Support.Fms.Gsfx.GsfxSurvey)
{
	MS.Support.Fms.Gsfx.GsfxSurvey = function (survey)
	{
		...
		var blessedDomains = {
		    "login.live.com": 420,
		    "microsoft.com": 300,
		    "search.microsoft.com": 300,
		    "windows.microsoft.com": 300,
		    "update.microsoft.com": 300,
		    "go.microsoft.com": 300,
		    "answerdesk.support.microsoft.com": 1200,
		    "windowsupdate.microsoft.com": 300,
		    "technet.microsoft.com": 300,
		    "download.microsoft.com": 300,
		    "answers.microsoft.com": 300,
		    "office.microsoft.com": 300,
		    "fixitcenter.support.microsoft.com": 300,
		    "office.com": 300,
		    "skype.com": 300,
		    "windowsphone.com": 300,
		    "xbox.com": 300,
		    "microsoftstore.com": 300,
		    "msdn.microsoft.com": 300,
		    "content.microsoftstore.com": 300
		};
		...
		function handleInvitationSubmit(survey, action)
		{
			...
			if (isPersist)
			{
				...
				{
					var surveywin = window.top.open(Replace(window.location.href, "surveyinvite.aspx", "survey.aspx"), "trackingWindow", "resizable=1,left=200,top=200,width=1024,height=700,scrollbars=1,status=1");

				    try{
				        if (window.sessionStorage) {
				            window.sessionStorage.tabSessionID = $.now();
				        }
				    } catch (e) { }
				...
				if ((!survey.isInvitation) && (!survey.isShowAll) && isOnUnloadSurvey)
				{
					...
					)
					{
						try
						{
							...
							if (ispostMessageSupportedBrowser() && isCookieEnabled()) {
							    ...
							    setCookie("P_STA", 1, null);
							    window.onbeforeunload = function () {
							        setCookie("P_STA", 0, null);
							    }
							    window.targetDomain = "none";
							}
							else {
							    startTracking();
							}
		...
		function receiveMessage(event) {			
		    if (event.origin == window.location.protocol + "//" + fmsDomain) {
		        surveyTimerTracking(event.data);
		    }
		}
		...
		function checkShowSurvey()
		{
		    try {
		        if (window.opener.closed == false) {		            
		            var openerLocation = $('<a>').prop('href', window.opener.location.href).prop('hostname').toLowerCase();
		            if (openerLocation.match(/^www\./)) {
		                openerLocation = openerLocation.substring(4);
		            }
		            if (openerLocation == fmsDomain) {
		                if (surveyTimer != null) {
		                    clearTimeout(surveyTimer);
		                }
		                window.targetDomain = "none";
		                if (globalCheck != 0) {
		                    window.setTimeout(checkShowSurvey, 100);
		                    --globalCheck;
		                }
		                return;
		            }
		...
		function surveyTimerTracking(location) {
		    try {
		        globalCheck = 0;
		        var navigatedDomain = $('<a>').prop('href', location).prop('hostname').toLowerCase();
		        if (navigatedDomain.match(/^www\./)) {
		            navigatedDomain = navigatedDomain.substring(4);
		        }
		        if (location == "ShowSurvey") {
		            showSurvey();
		        }
		        else if (location == "CheckShowSurvey") {
		            globalCheck = 100;
		            window.setTimeout(checkShowSurvey, 100);
		        }
		        else if ( navigatedDomain == fmsDomain) {
		            if (surveyTimer != null) {
		                clearTimeout(surveyTimer);
		            }
		            window.targetDomain = "none";
		        }
		        else if ($.inArray(navigatedDomain, Object.keys(blessedDomains)) != -1) {
		            var domainTimeout = blessedDomains[navigatedDomain] * 1000;
		            surveyTimer = window.setTimeout(surveyTimerTracking, domainTimeout, "ShowSurvey");
		            window.targetDomain = navigatedDomain;
		        }
		        else {
		            showSurvey();
		        }
