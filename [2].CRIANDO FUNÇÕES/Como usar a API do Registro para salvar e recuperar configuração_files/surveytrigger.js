...
if (!MS.Support.Fms.SurveyTrigger)
{
    MS.Support.Fms.SurveyTrigger = function(entity, config)
    {
        ...
        function isSMCDomain() {
            try {
                return (window.location.host.toLowerCase() == $('<a>').prop('href', $('script[src*="/surveytrigger"]').attr('src')).prop('hostname').toLowerCase()
        			|| $('<a>').prop('href', $('script[src*="/surveytrigger"]').attr('src')).prop('hostname') == "");
            } catch (e) { }
            return false;
        }

        if (isSMCDomain() && !window.crossDomainInitialized) {
            var version = "0";
            try {
                var temp = $('script[src*="/surveytrigger"]').attr('src');
                if (temp.indexOf("?") > -1)
            ...
            var src = protocol + "//" + fmsDomain + "/common/script/fx/" + header + "?" + version;
        ...
        function surveyDiv(fullURL)
        {
            ...
            else
            {
                ...
                function buildPluginTriggerInvitation()
                {
                    ...
                    if (isSMCDomain() && window.blessedDomains && typeof (JSON) != "undefined" && JSON.stringify) {
                        blessedDomainsScript = "<script type=\"text/javascript\">" + "window.blessedDomains =" + JSON.stringify(window.blessedDomains) + "</script>";
                    }
                    ...
                    var iframeHtml = "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">" +
											"<html>" +
												...
												"<body style=\"margin:0\">" +
													...
													"</div>" +
												"</body>" +
                                                "<script type=\"text/javascript\" src =\"" + config.protocol + "//ajax.aspnetcdn.com/ajax/jQuery/jquery-1.7.2.min.js\"></script>" +
												"<script type=\"text/javascript\" src =" + config.protocol + "//" + config.host + "/common/script/fx/surveystrapper.js?" + version + "></scr" + "ipt>" +
											"</html>";
