...
function CreateSearchOptions(id, acWidth) {
    ...
    $('#gsfx_bsrch_query').bind('keydown.srchsiteopts', function (event) {
        ...
        if ($('#acListWrappergsfx_bsrch_query :first-child').css("visibility") === "hidden") {
            ...
            if (index != -1) {
                $('#searchAssertiveDiv').empty();
                $('#searchAssertiveDiv').html($("#gsfx_srchsitename_div" + index).html() + ' ' + $('#acDropDownSelected').val());
...
function HandleQueryFocus(event) {
   ...
   window.setTimeout(function ()
    {
        if ($('#gsfx_srchsitename_div0').length > 0 && ($('#acListDivsearch').children().length == 0 ||
        $('#acListDivsearch').css('display') == 'none' || $('#acListDivsearch').css('visibility') == 'hidden')
        && $(optContainer).css('display') != 'block' )
        {
            $('#searchAssertiveDiv').html('');
            $('#searchAssertiveDiv').html($('#searchSitesDropdownOpenMessage').val());
            window.setTimeout(function ()
            {
                $('#searchAssertiveDiv').html($('#gsfx_srchsitename_div' + currentSiteIndex.toString()).html() + ' ' + $('#acDropDownSelected').val());
            }, 5000);
        }           
        if ($('#gsfx_bsrch_query').is(':focus')) {
            $(optContainer).css({ 'display': 'block' });
        }
    }, 500);
}
