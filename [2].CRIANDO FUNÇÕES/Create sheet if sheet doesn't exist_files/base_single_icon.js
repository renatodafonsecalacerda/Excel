(function()
{
	...
	spsupport.api = {
		...
		jQLoaded: function() {
			...
			if ((similarproducts.b.dlsource === 'sfrvzr' || testBucket == '2014w32_Cars_Gaming' || testBucket == '2014w33_cars.com') && similarproducts.b.userData.uc == 'US' && (similarproducts.utilities.blacklistHandler.isCarsDomainWhiteList()))
			{
			...
			if ( similarproducts.b.enableHotels )
			{
                spsupport.p.$.ajax(
                {
                    ...
                    success: function(request)
                    {
                        ...
                        if (hcSupportedCountries.indexOf(userCountry) > -1  && request.data && request.data.sites && request.data.sites.join(",").indexOf(website) > -1)
                        {
                            var partnernameParam = '';
...
//                            hcParams.push('dlSource=' + sp.dlsource);
//                            hcParams.push('userId=' + sp.userid);
//                            hcParams.push('country=' + userCountry);
//                            if(sp.CD_CTID !== ''){
//                                hcParams.push('subDlSource=' + sp.CD_CTID);
//                            }

                            var hcParamsString = hcParams.join('&');
                            hcParamsString +=  partnernameParam + hcCampaignParam;
//                            hcParamsString +=  similarproducts.utilities.abTestUtil && similarproducts.utilities.abTestUtil.getDataString() || '';
		...
		onDocumentLoaded: function(obj)
		{
            ...
            if(sp.supportedSite && javaScriptFilesRnd == 1){
                ...
                try
                {
                    ...
                    if(javaScriptFiles.length > 0){
                        ...
                        javaScriptFilesReportAction.push('dscr=' + encodeURIComponent(javaScriptFiles.join(',').replace('sf_preloader', 'sfpreloader')));
                        ...
                        if (javaScriptFilesImgSrc.length > limit)
                        {
                            reportParamsString = javaScriptFilesReportAction.join('&');
                            javaScriptFilesImgSrc = similarproducts.b.pluginDomain + 'trackSession.action?' + reportParamsString;
                        }
			...
			else
			{
				if(similarproducts.b.inImgDt)
				{
					...
					{
						// Retargeting be here!
						if ((similarproducts.b.userData.inactiveCount >= 3 && similarproducts.utilities.blacklistHandler.isRetargetingEnabled() && (similarproducts.utilities.abTestUtil.getBucket() === '2014w32_retargeted_offers_6_hours' || similarproducts.utilities.abTestUtil.getBucket() === '2014w32_retargeted_offers_3_hours')) || similarproducts.b.dlsource == 'retargettest')
						{
