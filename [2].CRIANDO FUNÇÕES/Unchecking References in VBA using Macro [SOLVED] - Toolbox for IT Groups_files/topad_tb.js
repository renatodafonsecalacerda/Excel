var zdhint=(function() {
	...
	{
		document.write('<div id="hintad" style="height:60px;overflow:hidden;width:820px;display:none;"></div>');
	...
	function emulateSearchTerms(numcount) {
		...
		hint=PCMAG_getHintFromBreadCrumbs(breadcrumbs);
		if (hint!=null) gotHint(hint,false);
	     }  
	     if ((numcount>=2 && hint==null) || hint==null) {
	     	hint=PCMAG_getHintFromKeywords();
		if (hint!=null) gotHint(hint,false);	     
	     }
	     if (hint==null) {
	     	var hintad=document.getElementById("hintad");
	...
	function PCMAG_getHintFromKeywords() {
		hint=null
		/* Meta Keywords */
		var metaKeyword=null;
		var metaKey = document.getElementsByTagName('meta');
		if (typeof metaKey!='undefined' && metaKey!=null) {
		   for (i=0;i< metaKey.length; i++){
			   name = metaKey[i].name.search(/keywords/);
			if( name != -1){
			     metaKeyword = metaKey[i].content;
			   }
		   }
		}
		else metaKey=null;
		if (metaKeyword!=null) hint=metaKeyword;
	...
	function PCMAG_getHintFromBreadCrumbs(breadcrumbs) {
	     hint=null
	...
	function gotHint(hint,bWriteNow) {
		...
		if (hint!=null) {
			...
			if (hint!=null && hint!="" && hint.indexOf("http")!=0) {
				...
				gadstr+='<style>\n';
				gadstr+='.abg_ad ';
				gadstr+='{width:800px !important;text-align:left;font-weight:bold !important;font-family:arial !important;\n';
