jQuery(document).ready(function($) {
	...
	var filters         = [];
	var pending_filters = [];
	filters.product     = wds_ms_get_active_filters( 'product' );
	filters.audience    = wds_ms_get_active_filters( 'audience' );
	filters.type        = wds_ms_get_active_filters( 'type' );
	...
	$('.main-navigation').on( 'click', '.main-navigation-top-level > li > a', function(e){
		...
		if ( $li.hasClass('open') ){
			$('.applied-filters').css( 'top', $li.children('.term-filters').outerHeight() ).addClass('down');
		} else {
			$('.applied-filters').css( 'top', 0 ).removeClass('down');
		}
	});
	...
	function wds_ms_update_filter_apply_link() {
		...
		// Update link href
		$('.main-navigation a.term-filters-apply').attr( 'href', filter_string );
	}
